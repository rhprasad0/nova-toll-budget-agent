# Saved release plans are isolated from application data.  A later trusted
# uploader must send ChecksumAlgorithm=SHA256 and use HeadObject to verify the
# returned version, stored checksum, KMS metadata, and default retention before
# applying a plan; S3 has no bucket-policy condition key for that checksum.
resource "aws_kms_key" "release_plan" {
  description             = "Nova Toll saved release plans"
