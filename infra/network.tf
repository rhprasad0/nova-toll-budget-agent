...
# NAT traffic keeps its private source until the router masquerades it.
resource "aws_vpc_security_group_ingress_rule" "router_nat_https" {
  for_each = toset([
  ...
  security_group_id = aws_security_group.tailscale_router.id
  description       = "Private subnet HTTPS egress through the router"
