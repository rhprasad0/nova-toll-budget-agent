  ...
  try {
    ...
    const checkChart = async () => {
      ...
      for (const bar of bars) {
        assert.equal(bar.usd, /** @type {import("../agent/assets/costs.mjs").Day} */ (payload.daily.find(row => row.date === bar.date))[/** @type {"aws" | "openai"} */ (bar.provider)]); assert.ok(bar.height >= 0);
        const index = bar.provider === 'aws' ? 0 : 1;
        assert.equal(await page.locator('#daily-rows tr').filter({ hasText: /** @type {string} */ (bar.date) }).locator('td').nth(index).getAttribute('data-usd'), bar.usd);
      }
    };
    await checkChart();
    fail = true;
