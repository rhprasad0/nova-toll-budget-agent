...
(async () => {
  ...
  const fixtures = {};
  for (const env of ['production', 'development']) fixtures[env] = JSON.parse(await fs.readFile(path.join(__dirname, `fixtures/costs-${env}.json`), 'utf8'));
  const routing = {};
  ...
  try {
    ...
    let environment = 'production', payload = fixtures.production, fail = false;
    ...
    assert.match(await page.locator('#state-notice').innerText(), /by design/);
    await checkChart();
    ...
    payload.month_to_date.aws = '-0.000000017'; payload.daily.forEach(row => { row.aws = '-0.000000001'; });
    ...
    payload.sources.aws_development.retrieved_at = payload.published_at;
    payload.sources.aws_development.daily.forEach((row, i) => { row.date = new Date(Date.parse(start) + i * 86400000).toISOString().slice(0, 10); });
