-- Add least-privilege I-95/I-495 current pricing comparisons.
...
BEGIN
    ...
    IF current_version NOT IN ('1.4.0', '1.5.0') THEN
        RAISE EXCEPTION 'expected oracle schema version 1.4.0 or 1.5.0, got %',
            current_version;
    END IF;
    IF to_regclass('pricing.i95_i495_pricing_comparisons') IS NULL THEN
        RAISE EXCEPTION 'oracle 1.5.0 requires I-95/I-495 pricing comparisons';
    END IF;
END
$migration$;

SELECT version = '1.4.0' AS oracle_upgrade_needed
FROM oracle.schema_version
WHERE singleton
\gset
...
GRANT SELECT ON pricing.i95_i495_pricing_comparisons TO oracle_owner;
...
REVOKE ALL ON FUNCTION oracle.get_i95_i495_pricing_comparisons(integer)
FROM PUBLIC;
...
SET version = '1.5.0', installed_at = clock_timestamp()
WHERE singleton AND version = '1.4.0';
...
BEGIN
    IF (SELECT version FROM oracle.schema_version WHERE singleton) <> '1.5.0' THEN
        RAISE EXCEPTION 'oracle 1.5.0 is not installed';
    END IF;
    ...
    IF pricing_function.provolatile <> 's'
       ...
       OR has_table_privilege(
        ...
        RAISE EXCEPTION 'I-95/I-495 pricing security contract is not installed';
    END IF;
END
$migration$;
