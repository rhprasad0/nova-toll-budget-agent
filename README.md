...
| --- | --- | --- | --- |
| Separate development and production environments | [Development backend](infra/backend.development.hcl), [production backend](infra/backend.production.hcl), and [account contract](infra/account-contract.json) | [Foundation contract tests](v2/tests/test_infrastructure_foundation.py) and [delivery documentation](v2/README.md#verify-the-build) | Separate account/state configuration; release evidence is specific to an artifact and run. |
...
That work exposed 330 pricing IDs in the I-95/I-495 route map but only 314 in retained VDOT history. The missing 16 affected 107 of 685 published routes. The [proxy mapping](v2/db/analysis.sql) uses source-overlap evidence to select available price proxies and labels those prices as modeled. The [historical holdout summary](v2/eval/EXPERIMENT_JOURNAL.md#i-95i-495-identity-proxy-validation) documents their measured error and limitations.
