...
**The 24-case golden corpus and calibrated judge contract are approved; a qualified production baseline and before/after application measurements remain outstanding.** The scheduled suite covers six current-toll scenarios with simulated users, a deterministic tool-call count check, and model-based completeness and correctness judges. The broader regression catalog checks parameters, clarification, route availability, money, and grounding. The reports below are scoped experiments, not a current whole-agent accuracy score or evidence of improvement between agent versions.

| Experiment | Recorded result | Scope and limits |
| --- | --- | --- |
| [AI evaluation engineering](v2/eval/GOLDEN_VALIDITY_REVIEW.md) | Judge agreement on 138/138 criterion labels across 46 development examples; simulated users passed 59/60 diagnostic checks | Human-reviewed evaluation contract; development calibration, not application accuracy. Production baseline remains outstanding. |
...
The 24-case golden corpus and calibrated judge contract are approved. The next milestone is a qualified production baseline, followed by comparisons of agent changes against that same contract. No application improvement percentage is claimed here.
